...
--]]
--[[
  Require from luvit edited for my needs
  See https://github.com/luvit/luvit/blob/master/deps/require.lua for original
]]
--[[lit-meta
  name = "luvit/require"
  version = "2.2.3"
...
local function moduleRequire(base, name)
  ...
  while true do
    if not skips[base] then
      local mod, path, key
      if isDir(pathJoin(base, "TypeWriter/PackageCache")) then
        mod, path, key = fixedRequire(pathJoin(base, "TypeWriter/PackageCache", name))
